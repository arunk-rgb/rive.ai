// splash-cursor.js — vanilla port of React Bits <SplashCursor/> (WebGL fluid cursor).
// window.initSplashCursor(options) -> { canvas, setActive(bool), destroy() }.
// Transparent overlay; meant to sit above another background and blend with it.
(function () {
  function pick(v, d) { return v === undefined ? d : v; }

  window.initSplashCursor = function (options) {
    options = options || {};
    const SIM_RESOLUTION = pick(options.SIM_RESOLUTION, 128);
    const DYE_RESOLUTION = pick(options.DYE_RESOLUTION, 1440);
    const CAPTURE_RESOLUTION = pick(options.CAPTURE_RESOLUTION, 512);
    const DENSITY_DISSIPATION = pick(options.DENSITY_DISSIPATION, 3.5);
    const VELOCITY_DISSIPATION = pick(options.VELOCITY_DISSIPATION, 2);
    const PRESSURE = pick(options.PRESSURE, 0.1);
    const PRESSURE_ITERATIONS = pick(options.PRESSURE_ITERATIONS, 20);
    const CURL = pick(options.CURL, 3);
    const SPLAT_RADIUS = pick(options.SPLAT_RADIUS, 0.2);
    const SPLAT_FORCE = pick(options.SPLAT_FORCE, 6000);
    const SHADING = pick(options.SHADING, true);
    const COLOR_UPDATE_SPEED = pick(options.COLOR_UPDATE_SPEED, 10);
    const BACK_COLOR = pick(options.BACK_COLOR, { r: 0.5, g: 0, b: 0 });
    const TRANSPARENT = pick(options.TRANSPARENT, true);
    const RAINBOW_MODE = pick(options.RAINBOW_MODE, true);
    const COLOR = pick(options.COLOR, '#ff0000');
    const zIndex = pick(options.zIndex, 50);
    const startHidden = pick(options.startHidden, false);
    ...
    Object.assign(canvas.style, {
      ...
      zIndex: String(zIndex), pointerEvents: 'none', display: 'block',
      opacity: startHidden ? '0' : '1', transition: 'opacity .4s ease',
    });
    document.body.appendChild(canvas);

    let isActive = true;               // false once destroyed
    let running = !startHidden;        // whether the sim loop runs
    let rafId = null;
    ...
    const gl = glctx.gl, ext = glctx.ext;
    if (!gl) { canvas.remove(); return { canvas: null, setActive() {}, destroy() {} }; }
    ...
    const baseVertexShader = compileShader(gl.VERTEX_SHADER, `
      ...
      varying vec2 vT; varying vec2 vB; uniform vec2 texelSize;
      void main () {
    ...
    const copyShader = compileShader(gl.FRAGMENT_SHADER, `
      ...
      uniform sampler2D uTexture; void main () { gl_FragColor = texture2D(uTexture, vUv); }`);
    const clearShader = compileShader(gl.FRAGMENT_SHADER, `
      precision mediump float; precision mediump sampler2D; varying highp vec2 vUv;
      uniform sampler2D uTexture; uniform float value;
      void main () { gl_FragColor = value * texture2D(uTexture, vUv); }`);
    const displayShaderSource = `
      precision highp float; precision highp sampler2D; varying vec2 vUv; varying vec2 vL;
      varying vec2 vR; varying vec2 vT; varying vec2 vB; uniform sampler2D uTexture;
      uniform sampler2D uDithering; uniform vec2 ditherScale; uniform vec2 texelSize;
      vec3 linearToGamma (vec3 color) { color = max(color, vec3(0));
    ...
    const splatShader = compileShader(gl.FRAGMENT_SHADER, `
      ...
      void main () { vec2 p = vUv - point.xy; p.x *= aspectRatio;
        vec3 splat = exp(-dot(p, p) / radius) * color; vec3 base = texture2D(uTarget, vUv).xyz;
        gl_FragColor = vec4(base + splat, 1.0); }`);
    const advectionShader = compileShader(gl.FRAGMENT_SHADER, `
      precision highp float; precision highp sampler2D; varying vec2 vUv; uniform sampler2D uVelocity;
      uniform sampler2D uSource; uniform vec2 texelSize; uniform vec2 dyeTexelSize; uniform float dt;
    ...
    const divergenceShader = compileShader(gl.FRAGMENT_SHADER, `
      precision mediump float; precision mediump sampler2D; varying highp vec2 vUv; varying highp vec2 vL;
      varying highp vec2 vR; varying highp vec2 vT; varying highp vec2 vB; uniform sampler2D uVelocity;
      void main () {
    ...
    const vorticityShader = compileShader(gl.FRAGMENT_SHADER, `
      precision highp float; precision highp sampler2D; varying vec2 vUv; varying vec2 vL; varying vec2 vR;
      varying vec2 vT; varying vec2 vB; uniform sampler2D uVelocity; uniform sampler2D uCurl;
    ...
    const pressureShader = compileShader(gl.FRAGMENT_SHADER, `
      precision mediump float; precision mediump sampler2D; varying highp vec2 vUv; varying highp vec2 vL;
      varying highp vec2 vR; varying highp vec2 vT; varying highp vec2 vB; uniform sampler2D uPressure;
    ...
    function updateKeywords() {
      let displayKeywords = []; if (config.SHADING) displayKeywords.push('SHADING');
    ...
    function updateFrame() {
      if (!isActive || !running) { rafId = null; return; }
      const dt = calcDeltaTime();
      if (resizeCanvas()) initFramebuffers();
      updateColors(dt); applyInputs(); step(dt); render(null);
      rafId = requestAnimationFrame(updateFrame);
    }
    function startLoop() { if (running && isActive && rafId == null) { lastUpdateTime = Date.now(); rafId = requestAnimationFrame(updateFrame); } }
    ...
    function resizeCanvas() {
      let width = scaleByPixelRatio(canvas.clientWidth), height = scaleByPixelRatio(canvas.clientHeight);
    ...
    function splatPointer(pointer) {
      let dx = pointer.deltaX * config.SPLAT_FORCE, dy = pointer.deltaY * config.SPLAT_FORCE;
      splat(pointer.texcoordX, pointer.texcoordY, dx, dy, pointer.color);
    }
    function clickSplat(pointer) {
      const color = generateColor(); color.r *= 10.0; color.g *= 10.0; color.b *= 10.0;
      let dx = 10 * (Math.random() - 0.5), dy = 30 * (Math.random() - 0.5);
    ...
    function splat(x, y, dx, dy, color) {
      ...
      gl.uniform1f(splatProgram.uniforms.radius, correctRadius(config.SPLAT_RADIUS / 100.0)); blit(velocity.write); velocity.swap();
    ...
    function hexToRGB(hex) {
      ...
      const r = parseInt(val.slice(0, 2), 16) / 255, g = parseInt(val.slice(2, 4), 16) / 255, b = parseInt(val.slice(4, 6), 16) / 255;
    ...
    function HSVtoRGB(h, s, v) {
      let r, g, b, i, f, p, q, t; i = Math.floor(h * 6); f = h * 6 - i;
      p = v * (1 - s); q = v * (1 - f * s); t = v * (1 - (1 - f) * s);
      switch (i % 6) {
        case 0: r = v; g = t; b = p; break; case 1: r = q; g = v; b = p; break;
        case 2: r = p; g = v; b = t; break; case 3: r = p; g = q; b = v; break;
        case 4: r = t; g = p; b = v; break; case 5: r = v; g = p; b = q; break; default: break;
      }
      return { r, g, b };
    }
    function wrap(value, min, max) { const range = max - min; if (range === 0) return min; return ((value - min) % range) + min; }
    function getResolution(resolution) {
      let a = gl.drawingBufferWidth / gl.drawingBufferHeight; if (a < 1) a = 1.0 / a;
    ...
    // input (gated on running so it only reacts while active/visible)
    ...
    function handleTouchEnd(e) { const touches = e.changedTouches; let pointer = pointers[0]; for (let i = 0; i < touches.length; i++) updatePointerUpData(pointer); }
